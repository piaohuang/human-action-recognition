...
def get_all_filename(video_dir, files_per_subdir=5):
    video_files = []
    for root, dirs, files in os.walk(video_dir):
        files_ = (random.sample(files, files_per_subdir) 
                        if len(files) > files_per_subdir 
                        else files)
        for f in files_:
...
class ContrastiveVideoDataset(Dataset):
    def __init__(self, video_dir, frame_size=224):
        self.video_files = get_all_filename(video_dir)
